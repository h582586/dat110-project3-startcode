...
public class MutualExclusion
{
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException
	{
		...
		boolean replyFromAll = areAllMessagesReturned(Util.numReplicas);
	...
	public void doDecisionAlgorithm(Message message, List<Message> queue, int condition) throws RemoteException
	{
		...
		case 2:
		{
			...
			int processClock = clock.getClock();

			// own clock for the multicast message
			int messageClock = message.getClock();

			// compare clocks, the lowest wins
			boolean senderLowest = (messageClock < processClock);

			// if clocks are the same, compare nodeIDs, the lowest wins
			if (messageClock == processClock)
			{
				senderLowest = (message.getNodeID().compareTo(node.getNodeID()) < 0);
			}
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException
	{
		...
		boolean messagesReturned = numvoters == queueack.size();

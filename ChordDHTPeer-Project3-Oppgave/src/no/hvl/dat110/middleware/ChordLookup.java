...
public class ChordLookup
{
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException
	{
		...
		NodeInterface keySuccessor = node.getSuccessor();
		
		// get the successor of the node
		
		// get the stub for this successor (Util.getProcessStub())
		NodeInterface stub = Util.getProcessStub(keySuccessor.getNodeName(), keySuccessor.getPort());
		
		// check that key is a member of the set {nodeid+1,...,succID} i.e. (nodeid+1 <= key <= succID) using the ComputeLogic
		BigInteger x = BigInteger.ONE;
		boolean isMemberOfSet = Util.computeLogic(key, node.getNodeID().add(x), keySuccessor.getNodeID());
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException
	{
		...
		for (int i = fingerTable.size() - 1; i >= 0; i--)
		{
			...
			boolean isMember = Util.computeLogic(fingerTable.get(i).getNodeID(), node.getNodeID().add(BigInteger.ONE), key.subtract(BigInteger.ONE));
